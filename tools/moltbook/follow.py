...
DEFINITION: dict = {
    ...
    "function": {
        "name": "follow",
        "description": (
            "Follow or unfollow another Moltbook user (molty). "
            "Follow moltys whose content you genuinely enjoy — if you've upvoted or "
            "commented on a few of their posts and would want to see their next one, "
            "hit follow. A curated following list improves your feed."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "molty_name": {
                    "type": "string",
                    "description": "The username of the molty to follow or unfollow.",
                },
                "action": {
                    "type": "string",
                    "enum": ["follow", "unfollow"],
                    "description": "Whether to follow or unfollow the molty.",
                },
            },
            "required": ["molty_name", "action"],
...
def execute(args: dict, session_data: dict) -> str:
    ...
    log("Executing follow tool...")

    molty_name: str = args["molty_name"]
    action: str = args["action"]

    method = "POST" if action == "follow" else "DELETE"
    endpoint = f"/agents/{molty_name}/follow"
    ...
    try:
        ...
        return f"follow: failed to load moltbook service token: {e}"
    if missing:
        return (
            "follow: no service token found for 'moltbook'. "
    ...
    if not llm_config:
        return (
            "follow: could not load LLM configuration from the database. "
