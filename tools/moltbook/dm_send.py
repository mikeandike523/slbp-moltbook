...
DEFINITION: dict = {
    ...
    "function": {
        "name": "dm_send",
        "description": (
            "Send a message in an active (approved) DM conversation. "
            "Set needs_human_input=true to flag that the other agent should escalate "
            "the message to their human rather than handle it themselves."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "conversation_id": {
                    "type": "string",
                    "description": "ID of the active conversation to send a message in.",
                },
                "message": {
                    "type": "string",
                ...
                "needs_human_input": {
                    "type": "boolean",
                    "description": (
                        "Set to true when the message requires a response from the other "
                        "agent's human, not just from the bot. The recipient will be "
                        "expected to escalate to their owner."
                    ),
                },
            },
...
def execute(args: dict, session_data: dict) -> str:
    ...
    log("Executing dm_send tool...")

    conversation_id: str = args["conversation_id"]
    message: str = args["message"]
    needs_human_input: bool | None = args.get("needs_human_input")

    body: dict = {"message": message}
    if needs_human_input is not None:
        body["needs_human_input"] = needs_human_input
    ...
    try:
        ...
        return f"dm_send: failed to load moltbook service token: {e}"
    if missing:
        return (
            "dm_send: no service token found for 'moltbook'. "
    ...
    if not llm_config:
        return (
            "dm_send: could not load LLM configuration from the database. "
    ...
    return run_mutation_loop(
        endpoint=f"/agents/dm/conversations/{conversation_id}/send",
        method="POST",
        llm=llm,
