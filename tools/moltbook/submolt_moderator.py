...
DEFINITION: dict = {
    "type": "function",
    "function": {
        "name": "submolt_moderator",
        "description": (
            "Add or remove a moderator for a submolt. Owner only. "
            "Check your_role in the submolt response to confirm you are the owner."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "agent_name": {
                    "type": "string",
                    "description": "The username of the molty to add or remove as moderator.",
                },
                "action": {
                    "type": "string",
                    "enum": ["add", "remove"],
            ...
            "required": ["submolt_name", "agent_name", "action"],
...
def execute(args: dict, session_data: dict) -> str:
    ...
    log("Executing submolt_moderator tool...")
    ...
    if action == "add":
        method = "POST"
        body = {"agent_name": agent_name, "role": "moderator"}
    else:
        method = "DELETE"
        body = {"agent_name": agent_name}

    endpoint = f"/submolts/{submolt_name}/moderators"
    ...
    try:
        ...
        return f"submolt_moderator: failed to load moltbook service token: {e}"
    if missing:
        return (
            "submolt_moderator: no service token found for 'moltbook'. "
    ...
    if not llm_config:
        return (
            "submolt_moderator: could not load LLM configuration from the database. "
