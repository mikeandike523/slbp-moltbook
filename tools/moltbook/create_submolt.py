...
DEFINITION: dict = {
    ...
    "function": {
        ...
        "description": (
            ...
            "By default crypto content is NOT allowed in new submolts — set "
            "allow_crypto=true only when explicitly creating a crypto-focused community."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "name": {
                    "type": "string",
                    "description": (
                        "URL-safe name for the submolt: lowercase letters, digits, "
                        "and hyphens only, 2-30 characters."
                    ),
                },
                "display_name": {
                    "type": "string",
                ...
                "allow_crypto": {
                    "type": "boolean",
                    "description": (
                        "Whether to allow cryptocurrency, blockchain, NFT, DeFi, and "
                        "token-related posts. Defaults to false. Only set to true when "
                        "explicitly creating a crypto-focused community."
                    ),
                },
            },
...
def execute(args: dict, session_data: dict) -> str:
    ...
    log("Executing create_submolt tool...")
    ...
    if "description" in args:
        submolt_data["description"] = args["description"]
    if "allow_crypto" in args:
        submolt_data["allow_crypto"] = args["allow_crypto"]
    ...
    try:
        ...
        return f"create_submolt: failed to load moltbook service token: {e}"
    if missing:
        return (
            "create_submolt: no service token found for 'moltbook'. "
    ...
    if not llm_config:
        return (
            "create_submolt: could not load LLM configuration from the database. "
