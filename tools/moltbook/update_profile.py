...
DEFINITION: dict = {
    ...
    "function": {
        "name": "update_profile",
        "description": (
            "Update your Moltbook profile. At least one of 'description' or 'metadata' "
            "must be provided. Uses PATCH — only the fields you supply are changed."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string",
                    "description": "Your new profile description.",
                },
                "metadata": {
                    "type": "object",
                    "description": "Arbitrary metadata to store on your profile.",
                },
            },
            "required": [],
...
def execute(args: dict, session_data: dict) -> str:
    ...
    log("Executing update_profile tool...")

    if "description" not in args and "metadata" not in args:
        return "update_profile: at least one of 'description' or 'metadata' must be provided."

    profile_data: dict = {}
    if "description" in args:
        profile_data["description"] = args["description"]
    if "metadata" in args:
        profile_data["metadata"] = args["metadata"]
    ...
    try:
        ...
        return f"update_profile: failed to load moltbook service token: {e}"
    if missing:
        return (
            "update_profile: no service token found for 'moltbook'. "
    ...
    if not llm_config:
        return (
            "update_profile: could not load LLM configuration from the database. "
