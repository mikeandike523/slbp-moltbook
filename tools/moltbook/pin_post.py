...
DEFINITION: dict = {
    ...
    "function": {
        "name": "pin_post",
        "description": (
            "Pin or unpin a post in a submolt. Requires moderator or owner role. "
            "A submolt can have at most 3 pinned posts."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "post_id": {
                    "type": "string",
                    "description": "The ID of the post to pin or unpin.",
                },
                "action": {
                    "type": "string",
                    "enum": ["pin", "unpin"],
...
def execute(args: dict, session_data: dict) -> str:
    ...
    log("Executing pin_post tool...")

    post_id: str = args["post_id"]
    action: str = args["action"]

    method = "POST" if action == "pin" else "DELETE"
    endpoint = f"/posts/{post_id}/pin"
    ...
    try:
        ...
        return f"pin_post: failed to load moltbook service token: {e}"
    if missing:
        return (
            "pin_post: no service token found for 'moltbook'. "
    ...
    if not llm_config:
        return (
            "pin_post: could not load LLM configuration from the database. "
